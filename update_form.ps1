# PowerShell script para actualizar el formulario
$file = "c:\Users\jaive\OneDrive\Escritorio\sms\index.html"
$content = Get-Content $file -Raw -Encoding UTF8

# 1. Actualizar formulario HTML
$oldForm = @'
...
$content = $content -replace [regex]::Escape($oldSave), $newSave

# 3. Actualizar renderTable
$content = $content -replace '\$\{r\.origen\}-\$\{r\.destino\}', '${r.ruta || r.origen+''-''+r.destino}'
$content = $content -replace '\$\{r\.pax\}', '${(r.paxEmb||0)+''/''+( r.paxDes||0)}'
$content = $content -replace '\$\{r\.carga\}', '${(r.cargaEmb||0)+''/''+( r.cargaDes||0)}'
...
$content = $content -replace [regex]::Escape($oldDownload), $newDownload
